...
void update(void)
{
    ...
    world_mat = mat4_mult_mat4(sm, world_mat);
    ...
    triangle[0] = mesh.vertices[mesh.faces[0].a];
    triangle[1] = mesh.vertices[mesh.faces[0].b];
    triangle[2] = mesh.vertices[mesh.faces[0].c];
    // printf("Vx: %f,, After: %f\n", triangle[0].x, (mat4_mult_vec4(world_mat, vec4_from_vec3(triangle[0])).x));


    // world_mat = mat4_mult_mat4(world_mat, sm);
    // world_mat = mat4_mult_mat4(world_mat, rmX);
    // world_mat = mat4_mult_mat4(world_mat, tm);
    for(int i = 0; i < n_faces; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            ...
            // transformed_vertex[j] = mat4_mult_vec4(rmX, transformed_vertex[j]);
            // transformed_vertex[j] = mat4_mult_vec4(rmY, transformed_vertex[j]);
            // transformed_vertex[j] = mat4_mult_vec4(rmZ, transformed_vertex[j]);
            // transformed_vertex[j] = mat4_mult_vec4(sm, transformed_vertex[j]);
            // transformed_vertex[j] = mat4_mult_vec4(tm, transformed_vertex[j]);
        ...
        for(int j = 0; j < 3; j++)
        {
            projection[j] = project(vec3_from_vec4(transformed_vertex[j]));
