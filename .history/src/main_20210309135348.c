...
void setup(void)
{
    ...
    mesh = Mesh_LoadObj("C:\\Users\\Hipreme\\Documents\\3dRenderer\\C\\assets\\teapot.obj");
}
...
void update(void)
{
    ...
    for(int i = 0; i < n_faces; i++)
    {
        ...
        //C - A = dirC
        //Normal = dir
        vec3 normal = vec3_cross(vec3_sub(vC, vA), vec3_sub(vB, vA));
        // vec3 normal = vec3_cross(vec3_sub(vB, vA), vec3_sub(vC, vA));
        vec3_normallize(&normal);
        // vec3_debug(normal);
        float color = vec3_dot(normal, (vec3){0.33, .33, 0.0});
        if(color < 0)
            color*=-1;
        color+= 0.2;
        color*=255;
        // color_u fColor = color_u_unpack_argb(currentFace.color);
        color_u fColor;
        fColor.r = color;
        fColor.g = color;
        fColor.b = color;
        fColor.a = color;
        uint32_t finalColor = color_u_pack_argb(fColor);
