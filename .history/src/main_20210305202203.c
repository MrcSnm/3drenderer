...
///Key C
void set_culling_enabled(void){PXRenderer_setCulling(PX_CULL_FRONT);print("%d", pxRenderer.cullType);}
///Key D
void set_culling_disabled(void){PXRenderer_setCulling(PX_CULL_NONE);print("%d", pxRenderer.cullType);}
...
void render(void)
{
    ...
    for(int i = 0; i < n_triangles; i++)
    {
        ...
        {
            pxDrawPixel(t.points[0].x, t.points[0].y, 0xFF0000);
            pxDrawPixel(t.points[1].x, t.points[1].y, 0xFF0000);
            pxDrawPixel(t.points[2].x, t.points[2].y, 0xFF0000);
