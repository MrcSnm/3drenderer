...
void update(void)
{
    ...
    for(int i = 0; i < N_CUBE_FACES; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            ...
            projection.x = (projection.x*fov_factor)+ pxBuffer.halfW;
            projection.y = (projection.y*fov_factor)+ pxBuffer.halfH;  
...
void render(void)
{
    ...
    for(int i = 0; i < N_CUBE_FACES; i++)
    {
        ...
        pxFillRect(t.points[0].x, t.points[0].y,4,4,0xFF00FF00);
        pxFillRect(t.points[1].x, t.points[1].y,4,4,0xFF00FF00);
        pxFillRect(t.points[2].x, t.points[2].y,4,4,0xFF00FF00);
    }
    pxRender();
