...
void update(void)
{
    ...
    mat4 rmX = mat4_translation_mat(mesh.rotation.x);
    mat4 rmY = mat4_translation_mat(mesh.rotation.y);
    mat4 rmZ = mat4_translation_mat(mesh.rotation.z);
    for(int i = 0; i < n_faces; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            ...
            transformed_vertex[j] = mat4_mult_vec4(rm, transformed_vertex[j]);
