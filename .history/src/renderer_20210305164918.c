...
static void _pxFillFlatBotTriangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
    ...
    float inc_x = dx/length;

    int dx2 = x2-x0;
    float inc_x2 = dx2/length;

    //Walk from (x0, y0) to (x1, y1)
    for(; y0 < y1 ;y0++)
    {
        pxDrawLine(round(x0t), y0, round(x2t), y0, color);//Draw horizontal lines
        x0t+= inc_x;
        x2t+= inc_x2;
    }
}
static void _pxFillFlatTopTriangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
    ...
    int dx = x0-x2; //Walk to x0
    float length = y2 - y1; //Y1 is always greater than y0
    float inc_x = dx/length;

    int dx2 = x1-x2;//Walk to x1
    float inc_x2 = dx2/length;

    //Walk from (x0, y0) to (x1, y1)
    for(; y2 > y0 ;y2--)
    {
        pxDrawLine(round(x0t), y2, round(x2t), y2, color);//Draw horizontal lines
        x0t+= inc_x;
        x2t+= inc_x2;
    }
}
