...
float fov_angle = DEGREES(60); //30 degrees
float zfar = 100;
float znear = 0.1;
mat4 proj_matrix;
...
void setup(void)
{
    ...
    proj_matrix = mat4_perspective_mat(pxBuffer.height/pxBuffer.width, fov_angle, znear, zfar);
...
void update(void)
{
    ...
    for(int i = 0; i < n_faces; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            ...
            projection[j].x *= pxBuffer.width / 2.0f;
            projection[j].y *= pxBuffer.height / 2.0f;
