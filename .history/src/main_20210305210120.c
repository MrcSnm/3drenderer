...
void update(void)
{
    ...
    for(int i = 0; i < n_faces; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            projection[i] = project(transformed_vertex[j]);
            //Scale and translate to the screen center
            projection[i].x+= pxBuffer.halfW;
            projection[i].y+= pxBuffer.halfH;  
        }
        triangle_t projectedTriangle = {.points = projection, .color = currentFace.color};
