...
#define profiling_start PASTE(debug) {  \
PASTE(debug) clock_t _prof_begin, _prof_end;\
PASTE(debug) _prof_begin = clock();\

#define profiling_end(name) PASTE(debug) _prof_end = clock(); \
PASTE(debug) print("Time spent on %s: %lf secs\n", name, (double)(_prof_end - _prof_begin)/CLOCKS_PER_SEC);\
PASTE(debug) }\
