...
bool culling(vec3 a, vec3 b, vec3 c, vec3 camPos)
{
    ...
    vec3 normal = Triangle_getNormal(a, b, c);
 ...
 * Clipping <- Clip against 6 frustum planes
 * Projection <- Multiply by projection matrix
 * Image space <- Perspective divide, NDC
 * Screen Space <- Render
*/
void process_graphics_pipeline(mesh_t* mesh)
{
    ...
    for(int i = 0; i < n_faces; i++)
    {
        ...
        uint32_t finalColor = color_argb_multiply(currentFace.color, lightRate, false);
...
void update(void)
{
    ...
    for(int mesh_index = 0; mesh_index < Array_length(game_meshes); mesh_index++)
    {
        mesh_t* mesh = &game_meshes[mesh_index];
